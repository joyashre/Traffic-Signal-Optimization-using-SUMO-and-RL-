...
def generate_report():
    ...
    # --- NAYA DETAILED ERROR CHECK ---
    files_ok = True
    if not os.path.exists('baseline_tripinfo.xml'): 
        print("❌ Missing: baseline_tripinfo.xml file hi nahi hai.")
        files_ok = False
    elif df_base.empty: 
        print("⚠️ Khali File: baseline_tripinfo.xml mili, par isme kisi gaadi ka data nahi hai (Simulation mein gaadiyan nahi chali).")
        files_ok = False

    if not os.path.exists('with_rl_tripinfo.xml'): 
        print("❌ Missing: with_rl_tripinfo.xml file hi nahi hai.")
        files_ok = False
    elif df_rl.empty: 
        print("⚠️ Khali File: with_rl_tripinfo.xml mili, par isme kisi gaadi ka data nahi hai.")
        files_ok = False
        
    if not files_ok:
        print("\n🛑 Execution rok diya gaya hai. Upar diye gaye errors check karein.")
        return
    # ---------------------------------
